...
\end{equation}
Dove $T_1$ è la temperatura dell'acqua contenuta nel calorimetro che, in approssimazione, si assume essere pari all'ultima temperatura registrata e $T_0$ corrisponde alla prima misura utile registrata.
...
\label{fig:fit_log}
\centering
\includegraphics[scale=0.7]{fit_log}
...
L'incertezza sulle misure di tempo, derivante dall'assunzione di una distribuzione uniforme dell'errore sull'ultimo digit, può, in approssimazione, essere trascurata. La temperatura è invece affetta, come osservato in precedenza, da un errore di $\SI{0.1}{\degree C}$ che andrà ad influire sull'incertezza associata ad i parametri del fit che si intende eseguire. Graficando i valori ottenuti dall'esperimento, figura \ref{fig:cal_t}, è evidente che la temperatura dell'acqua, nel calorimetro, decresce linearmente nel tempo. Si procede dunque all'esecuzione di un fit lineare che fornisce una stima dei parametri $A$ e $B$ dell'equazione regolatrice del processo\footnote{Intercetta e coefficiente angolare.}:\[
...
Non avendo misurato la massa d'acqua inserita nel calorimetro non è ora possibile utilizzare l'equazione\[
Q=mc\Delta T
\]
per determinare la quantità di calore ceduta ed il calore specifico del calorimetro.
